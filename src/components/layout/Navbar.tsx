    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 ${
          isScrolled
            ? 'bg-[#F7F4EE]/90 backdrop-blur-xl border-b border-[#EDE8DE] py-3.5 shadow-[0_4px_25px_rgba(0,132,200,0.04)]'
          ...
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden lg:flex items-center gap-1 xl:gap-1.5 px-3 py-1.5 rounded-full bg-[#EDE8DE]/90 border border-[#EDE8DE] backdrop-blur-md shadow-xs">
              {navLinks.map((link) => (
                <button
                  ...
                  className="px-4 py-2 rounded-full font-mono text-xs tracking-[0.18em] transition-all duration-300 text-[#171717]/80 hover:text-[#0084C8] hover:bg-white cursor-pointer font-medium"
                >
                  {link.name}
                </button>
              ))}
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="hidden sm:inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#0084C8] hover:bg-[#006EAA] text-white font-display font-bold text-xs uppercase tracking-wider transition-all duration-300 shadow-[0_4px_20px_rgba(0,132,200,0.3)] cursor-pointer group"
              >
          ...
          >
            ...
            <div className="my-auto py-10 flex flex-col gap-4">
              ...
              {navLinks.map((link, idx) => (
                <button
                  ...
                  className="font-display text-3xl sm:text-4xl font-extrabold uppercase tracking-tight flex items-center justify-between py-2 border-b border-[#EDE8DE] transition-colors text-[#171717] hover:text-[#0084C8] text-left cursor-pointer"
                >
                  <div className="flex items-center gap-4">
                    <span className="font-mono text-xs text-[#B8955A]">0{idx + 1}</span>
                    <span>{link.name}</span>
                  </div>
                  <ArrowUpRight className="w-6 h-6 text-[#0084C8]" />
                </button>
              ))}
            </div>
            ...
            <div className="pt-6 border-t border-[#EDE8DE] flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div className="flex flex-col gap-1">
                ...
                <span className="text-xs font-mono text-[#0084C8] font-medium">{companyData.contact.email}</span>
              </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex-1 sm:flex-initial px-6 py-3.5 rounded-full bg-[#0084C8] text-white font-display font-bold text-xs uppercase tracking-wider text-center cursor-pointer shadow-md"
                >
                  Request Quote
                </button>
                <a
