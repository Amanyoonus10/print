...
export const ServiceShowcases: React.FC<{ onOpenQuoteModal?: () => void }> = ({ onOpenQuoteModal }) => {
  ...
  return (
    <div id="services-showcases" className="w-full flex flex-col bg-[#F7F4EE]">
      {services.map((service, index) => {
        ...
        return (
          <section
            key={service.slug}
            id={service.slug}
            className={`relative py-24 md:py-32 border-b border-[#EDE8DE] overflow-hidden ${
              isEven ? 'bg-[#F7F4EE]' : 'bg-[#EDE8DE]/30'
            }`}
          >
            {/* Ambient Background Glow */}
            <div className={`absolute top-1/2 ${isEven ? 'left-0' : 'right-0'} -translate-y-1/2 w-96 h-96 bg-[#49C1DA]/5 rounded-full blur-3xl pointer-events-none`} />
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
                ...
                >
                  {/* Oversized Title */}
                  <h2 className="font-display font-extrabold text-4xl sm:text-5xl md:text-6xl text-[#171717] tracking-tight uppercase leading-[1.05]">
                    {service.title}
                  </h2>

                  {/* Subtitle / Statement */}
                  <p className="text-lg md:text-xl font-medium text-[#49C1DA] leading-snug">
                    “{service.subtitle}”
                  </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-2">
                    {service.features.slice(0, 4).map((feature, fIdx) => (
                      <div key={fIdx} className="flex items-start gap-2.5">
                        ...
                        <span className="text-xs font-mono text-[#171717] font-semibold">
                          {feature}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="flex flex-wrap items-center gap-4 pt-4">
                    {onOpenQuoteModal ? (
                      <button
                        ...
                        className="px-6 py-3.5 rounded-full bg-[#49C1DA] hover:bg-[#32AEC8] text-white font-display font-bold text-xs uppercase tracking-wider flex items-center gap-2 transition-all duration-300 shadow-[0_4px_20px_rgba(73,193,218,0.3)] cursor-pointer group hover:scale-102"
                      >
                      ...
                      <a
                        ...
                        className="px-6 py-3.5 rounded-full bg-[#49C1DA] hover:bg-[#32AEC8] text-white font-display font-bold text-xs uppercase tracking-wider flex items-center gap-2 transition-all duration-300 shadow-[0_4px_20px_rgba(73,193,218,0.3)]"
                      >
                        <span>Inquire {service.title}</span>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-6 py-3.5 rounded-full bg-white hover:bg-[#EDE8DE] text-[#171717] font-mono text-xs uppercase tracking-wider border border-[#EDE8DE] transition-all font-semibold shadow-xs"
                    >
                      WhatsApp Specs
                    </a>
                  </div>
